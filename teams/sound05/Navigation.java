...
public class Navigation {
	private static void cleverMove(RobotController rc, Direction d)throws GameActionException{
		...
		if(rc.getLocation().distanceSquaredTo(enemyHQLoc)<= rc.getType().attackRadiusMaxSquared+9 ){
			// we're close to the HQ shooting us.
			//rc.move(d.opposite());
			return;
		}
	...
	public static Direction wonder(RobotController rc, Random rand, Direction d) throws GameActionException {
		...
		if((d==danger) && (rc.getLocation().distanceSquaredTo(enemyHQ)<= rc.getType().attackRadiusMaxSquared+1 )){
			d=d.opposite();
		}
